...
contract LP is Test {
    ...
    constructor(IERC20 _token0, IERC20 _token1){
        ...
        token0.approve(address(univ2router), type(uint).max);
        token1.approve(address(univ2router), type(uint).max);
    }

    function add(uint amt0, uint amt1) public {
        univ2router.addLiquidity(address(token0), address(token1), amt0, amt1, 0, 0, address(this), type(uint).max);
    ...
    function remove() public {
        ...
        pair.approve(address(univ2router), type(uint).max);
