...
contract ZapTest_fork is Test {
    ...
    function test_fork_stakeLP(bool token0, bool isImbalanced, bool imbalanceDirection) public {
        ...
        if (isImbalanced) {
            emit log_named_uint("balanceOF", lpLp.t0().balanceOf(address(lpLp)));
            emit log_named_uint("balanceOF", lpLp.t1().balanceOf(address(lpLp)));
            lpLp.trade(10 ether, imbalanceDirection);
        }

        lpF0.transfer(address(treasury), 1 ether);
